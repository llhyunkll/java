...
- 열거형의 속성 및 메서드 
  ...
  ordinal() 	==> 열거형 상수가 정의된 순서값(index값)을 반환한다. (index는 0부터 시작)
...
방법2)
	enum 열거형이름 { 
		...
		// 구성된 '값들'을 외부에서 불러올 수 있는 getter매서드를 만든다.
	...
	public static void main(String[] args) {
		...
		*/
		...
		for(Season time : Season.values()){
			System.out.println(time + " === " + time.name() + " ==> " + time.getSpan()
					+ ", " +time.getNum());
		}
