...
class ShareData{
	...
	// volatile ==> 이 키워드가 붙은 변수는 컴파일러의 최적화 대상에서 제외된다. //속도는 더 느리지만 메모리에 있는 변수를 가져오고 저장함
	// 				즉, 값이 변경되는 즉시 변수에 적용시킨다.
	public volatile boolean isOk;	// 계산이 완료되었는지 여부를 나타내는 변수 (계산이 완료되면 true가 된다)
...
class CalcPIThread extends Thread {
	...
	public void run() {
		 ...
		 */
		double sum = 0.0;
		for(int i = 1; i <= 1_000_000_000; i+=2){ //반복하는 횟수가 많아질수록 제공되는 PI값과 가까워짐
			if( (i / 2) % 2 == 0 ){ // 짝수번째
				sum += (1.0 / i); 
			}else{
				sum -= (1.0 / i);
