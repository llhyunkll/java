...
public class SetTest01 {
	...
	public static void main(String[] args) {
		...
		System.out.println("Set 데이터 : " + hs1); // 입력한 순서와 출력되는 순서가 일치하지 않음
		...
		System.out.println("Set 데이터 : " + hs1); // 데이터 추가되지 않음
		
		// Set의 데이터를 수정하려면 수정한느 명령이 따로 없기 때문에
...
//		hs1.clear();
//		System.out.println("clear 후 Set => " + hs1); //데이터 전부 삭제됨
		...
		/*
			Set의 데이터 순서(index)가 없기 때문에 List처럼 index로 데이터를 하나씩 불러올 수 없다.
		...
		// Iterator의 hasNext()메서드 ==> Iterator의 데이터를 가리키는 포인터의 다음번째의 데이터 위치에
...
//		System.out.println(Arrays.toString(numArr));
		...
		while(numSet.size() < 3){
			numSet.add((int)(Math.random()*25 + 1)); //set은 중복된 데이터를 저장하지 않으니까...
		}
