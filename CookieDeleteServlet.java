...
public class CookieDeleteServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//   형식) cookie객체변수.setMaxAge(시간); // 시간은 밀리세컨드(1/1000초) 단위로 지정한다. 
		// 		  ==> 시간이 0이면 쿠키가 바로 삭제되고 
		// 		  ==> 시간이 음수이면 웹 브라우저가 끝날 때 쿠키가 삭제된다. 
		// 		  ==> 시간이 양수이면 '지정된 시간'동안 저장된 후 지정된 시간이 경과하면 자동으로 삭제된다.
		
		// 예제) 쿠키 변수가 'name'인 쿠키 삭제하기
		...
		if(cookieArr == null || cookieArr.length == 0){
			out.println("<h2>저장된 Cookie정보가 하나도 없습니다.</h2>");
		}
		...
		for(Cookie cookie : cookieArr){
			...
			if(name.equals("name")){ // 삭제할 쿠키변수인지 확인
				...
				nameCookie.setMaxAge(0);	// 유지시간을 0으로 변경한다. 설정하지 않으면 웹브라우저 끝날때까지 유지됨
				response.addCookie(nameCookie);
			}
		}
		
		out.println("<a href='"+request.getContextPath() + 
				"/basic/02/cookieTest.jsp'>시작 문서로 이동</a>");
